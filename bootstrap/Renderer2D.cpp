...
//BEGIN WITH SCALE IMPLEMENTATION
void Renderer2D::begin() {
	...
	// apply camera position and scale to projection
	float scaledWidth = (float)width * m_cameraScale;
	float scaledHeight = (float)height * m_cameraScale;

	float midX = m_cameraX + (float)width / 2.0f;
	float midY = m_cameraY + (float)height / 2.0f;
